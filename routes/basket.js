...
router.post("/save-to-basket", async (req, res, next) => {
  ...
  if (duplicated) {
    ...
    Basket.findOneAndUpdate({ userId: userId }, { $push: { basket: item } })
      .then((response) => {
        console.log("added to basket");
        res
          .status(200)
          .json({
            res: { products: response.basket, message: "added to basket" },
          });
      })
      .catch((error) => {
...
router.post("/remove-from-basket", async (req, res, next) => {
  const { userId, item } = req.body;
  const removeingItem = await Basket.update(
    { userId: userId },
    { $pull: { basket: { _id: item._id } } }
  );
  if (removeingItem) {
